...
#ifndef CHUNKED_RUNTIME_UTILS_EXPRS_EXPR_PARSER_H_
#define CHUNKED_RUNTIME_UTILS_EXPRS_EXPR_PARSER_H_
...
#include "flex/engines/graph_db/chunked_runtime/utils/exprs/map_expr.h"
#include "flex/engines/graph_db/chunked_runtime/utils/exprs/tuple.h"
#include "flex/engines/graph_db/chunked_runtime/utils/exprs/udf.h"
...
#include "flex/engines/graph_db/chunked_runtime/utils/exprs/with_in_expr.h"
...
class ExprParser {
 public:
  static inline RTAny parse_const_value(const common::Value& val) {
  ...
  static inline RTAny parse_param(
      const common::DynamicParam& param,
      const std::map<std::string, std::string>& input) {
    if (param.data_type().type_case() ==
        common::IrDataType::TypeCase::kDataType) {
      auto type = gs::runtime::parse_from_ir_data_type(param.data_type());
  ...
  struct TypedTupleBuilder {
    std::unique_ptr<ExprBase> build_typed_tuple(
      ...
      case RTAnyType::kI64Value:
        return TypedTupleBuilder<N, I - 1, int64_t, Args...>()
  ...
  static std::unique_ptr<ExprBase> parse_expression_impl(
      const GraphInterface& graph, const DataChunk& ctx,
    ...
    while (!opr_stack.empty()) {
        ...
        if (opr.logical() == common::Logical::WITHIN) {
          ...
          if (rhs.has_const_()) {
            ...
            if (key->type() == RTAnyType::kI64Value) {
              return std::make_unique<WithInExpr<int64_t>>(std::move(key),
                                                           rhs.const_());
            } else if (key->type() == RTAnyType::kU64Value) {
              return std::make_unique<WithInExpr<uint64_t>>(std::move(key),
                                                            rhs.const_());
            } else if (key->type() == RTAnyType::kI32Value) {
              return std::make_unique<WithInExpr<int32_t>>(std::move(key),
                                                           rhs.const_());
            } else if (key->type() == RTAnyType::kStringValue) {
              return std::make_unique<WithInExpr<std::string>>(std::move(key),
                                                               rhs.const_());
            } else {
            ...
            if (key->type() == RTAnyType::kVertex) {
              auto val = std::make_unique<VariableExpr>(graph, ctx, rhs.var(),
                                                        var_type);
              if (val->type() == RTAnyType::kList) {
                return std::make_unique<VertexWithInListExpr>(std::move(key),
                                                              std::move(val));
              } else if (val->type() == RTAnyType::kSet) {
                return std::make_unique<VertexWithInSetExpr>(std::move(key),
                                                             std::move(val));
              } else {
                LOG(FATAL) << "not support";
              }
      ...
      case common::ExprOpr::kExtract: {
        ...
        if (hs->type() == RTAnyType::kI64Value) {
          ...
          return std::make_unique<ExtractExpr<Day>>(std::move(hs),
                                                    opr.extract());
      ...
      case common::ExprOpr::kVars: {
        ...
        if (op.keys_size() == 3) {
          ...
          for (int i = 0; i < op.keys_size(); ++i) {
            exprs[i] = std::make_unique<VariableExpr>(graph, ctx, op.keys(i),
                                                      var_type);
          }
      ...
      case common::ExprOpr::kMap: {
        ...
        if (exprs.size() > 0) {
          return std::make_unique<MapExpr>(std::move(keys_vec),
                                           std::move(exprs));
        }
      ...
      case common::ExprOpr::kUdfFunc: {
        ...
        auto expr = parse_expression_impl(graph, ctx, params, op.parameters(0),
                                          var_type);
        if (name == "gs.function.relationships") {
          ...
          auto expr2 = parse_expression_impl(graph, ctx, params,
                                             op.parameters(1), var_type);
  ...
  template <typename GraphInterface>
  static std::unique_ptr<ExprBase> parse_expression_impl(
      const GraphInterface& graph, const DataChunk& chunk,
      const std::map<std::string, std::string>& params,
      const common::Expression& expr, VarType var_type) {
    ...
    return build_expr(graph, chunk, params, opr_stack2, var_type);
  }
};
template <typename GraphInterface>
static std::unique_ptr<ExprBase> parse_expression(
    const GraphInterface& graph, const DataChunk& chunk,
    const std::map<std::string, std::string>& params,
    const common::Expression& expr, VarType var_type) {
  return ExprParser::parse_expression_impl(graph, chunk, params, expr,
                                           var_type);
}
}  // namespace chunked_runtime
}  // namespace gs
#endif  // CHUNKED_RUNTIME_UTILS_EXPRS_EXPR_PARSER_H_