...
Var::Var(const GraphInterface& graph, const DataChunk& ctx,
         const common::Variable& pb, VarType var_type)
    : getter_(nullptr) {
  int tag = -1;
  type_ = RTAnyType::kUnknown;
  if (pb.has_node_type()) {
    type_ = gs::runtime::parse_from_ir_data_type(pb.node_type());
  }
  if (pb.has_tag()) {
    tag = pb.tag().id();
  }
    ...
    if (ctx.get(tag)->column_type() == ContextColumnType::kVertex) {
      if (pb.has_property()) {
        ...
        if (pt.has_id()) {
          ...
          getter_ = gs::chunked_runtime::create_vertex_property_path_accessor(
              graph, ctx, tag, type_, pt.key().name());
        } else if (pt.has_label()) {
          getter_ =
              gs::chunked_runtime::create_vertex_label_path_accessor(ctx, tag);
        } else {
      ...
      getter_ =
          gs::chunked_runtime::create_context_value_accessor(ctx, tag, type_);
    } else if (ctx.get(tag)->column_type() == ContextColumnType::kEdge) {
      if (pb.has_property()) {
        auto& pt = pb.property();
        if (pt.has_key()) {
          auto name = pt.key().name();
          getter_ = gs::chunked_runtime::create_edge_property_path_accessor(
              graph, name, ctx, tag, type_);
        } else if (pt.has_label()) {
          getter_ =
              gs::chunked_runtime::create_edge_label_path_accessor(ctx, tag);
        } else {
    ...
    if (var_type == VarType::kVertexVar) {
      if (pb.has_property()) {
        ...
        if (pt.has_id()) {
          ...
          getter_ = gs::chunked_runtime::create_vertex_property_vertex_accessor(
              graph, type_, pt.key().name());
      ...
      if (pb.has_property()) {
        ...
        if (pt.has_key()) {
          auto name = pt.key().name();
          getter_ = gs::chunked_runtime::create_edge_property_edge_accessor(
              graph, name, type_);
        } else {
          LOG(FATAL) << "parse failed for " << pt.DebugString();
        }
...
template Var::Var(const GraphReadInterface& graph, const DataChunk& ctx,
                  const common::Variable& pb, VarType var_type);
template Var::Var(const GraphUpdateInterface& graph, const DataChunk& ctx,
                  const common::Variable& pb, VarType var_type);
