...
std::shared_ptr<IAccessor> create_context_value_accessor(const DataChunk& ctx,
                                                         int tag,
                                                         RTAnyType type) {
  switch (type) {
...
std::shared_ptr<IAccessor> create_vertex_property_path_accessor(
    const GraphInterface& graph, const DataChunk& ctx, int tag, RTAnyType type,
...
std::shared_ptr<IAccessor> create_edge_property_path_accessor(
    const GraphInterface& graph, const std::string& name, const DataChunk& ctx,
    int tag, RTAnyType type) {
  auto col = dynamic_cast<const IEdgeColumn*>(ctx.get(tag));
...
template std::shared_ptr<IAccessor> create_vertex_property_path_accessor(
    const GraphReadInterface& graph, const DataChunk& ctx, int tag,
    RTAnyType type, const std::string& prop_name);
template std::shared_ptr<IAccessor> create_vertex_property_path_accessor(
    const GraphUpdateInterface& graph, const DataChunk& ctx, int tag,
    RTAnyType type, const std::string& prop_name);
...
template std::shared_ptr<IAccessor> create_edge_property_path_accessor(
    ...
    const DataChunk& ctx, int tag, RTAnyType type);
template std::shared_ptr<IAccessor> create_edge_property_path_accessor(
