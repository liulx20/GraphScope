...
class TCOpr : public IReadOpr {
 public:
  TCOpr(std::unique_ptr<IReadOpr>&& src_opr,
        const physical::EdgeExpand& ee_opr0,
    ...
    dir0_ = gs::runtime::parse_direction(ee_opr0.direction());
    dir1_ = gs::runtime::parse_direction(ee_opr1.direction());
    dir2_ = gs::runtime::parse_direction(ee_opr2.direction());
    alias1_ = -1;
  ...
  bl::result<void> EvalChunks(const gs::runtime::GraphReadInterface& graph,
    ...
    auto& casted_state = dynamic_cast<TCState&>(state);
    casted_state.initialize(input_tag_, alias1_, alias2_);
    return ForEachChunk(chunks, [&](const DataChunk& chunk) {
      auto& local_state = casted_state.getLocalTCState();
      const std::string& param_value = params.at(param_name_);
      return EdgeExpand::tc<T1, T2, T3>(graph, chunk, labels_, input_tag_,
...
bool tc_fusable(const physical::EdgeExpand& ee_opr0,
  ...
  auto sp_pred = gs::runtime::parse_sp_pred(ee_opr0.params().predicate());
  if (sp_pred != gs::runtime::SPPredicateType::kPropertyGT &&
      sp_pred != gs::runtime::SPPredicateType::kPropertyLT) {
    return false;
  }
...
std::unique_ptr<IReadOpr> _make_tc_opr(
    std::unique_ptr<IReadOpr>&& src_opr, const physical::EdgeExpand& ee_opr0,
    const physical::EdgeExpand& ee_opr1, const physical::GetV& v_opr1,
    const physical::EdgeExpand& ee_opr2, const LabelTriplet& label0,
    const LabelTriplet& label1, const LabelTriplet& label2,
    const std::array<PropertyType, 3>& eps) {
  if (eps[2] == PropertyType::Empty()) {
    return std::make_unique<TCOpr<T1, T2, grape::EmptyType>>(
        std::move(src_opr), ee_opr0, ee_opr1, v_opr1, ee_opr2, label0, label1,
        label2);
  } else if (eps[2] == PropertyType::Date()) {
    return std::make_unique<TCOpr<T1, T2, Date>>(std::move(src_opr), ee_opr0,
                                                 ee_opr1, v_opr1, ee_opr2,
                                                 label0, label1, label2);
  } else if (eps[2] == PropertyType::Int64()) {
    return std::make_unique<TCOpr<T1, T2, int64_t>>(std::move(src_opr), ee_opr0,
                                                    ee_opr1, v_opr1, ee_opr2,
                                                    label0, label1, label2);
  }
  return nullptr;
...
std::unique_ptr<IReadOpr> make_tc_opr(
    std::unique_ptr<IReadOpr>&& src_opr, const physical::EdgeExpand& ee_opr0,
    const physical::EdgeExpand& ee_opr1, const physical::GetV& v_opr1,
    const physical::EdgeExpand& ee_opr2, const LabelTriplet& label0,
    const LabelTriplet& label1, const LabelTriplet& label2,
  ...
  if (eps[0] == PropertyType::Date()) {
    if (eps[1] == PropertyType::Date()) {
      return _make_tc_opr<Date, Date>(std::move(src_opr), ee_opr0, ee_opr1,
                                      v_opr1, ee_opr2, label0, label1, label2,
                                      eps);
    } else if (eps[1] == PropertyType::Empty()) {
      return _make_tc_opr<Date, grape::EmptyType>(std::move(src_opr), ee_opr0,
                                                  ee_opr1, v_opr1, ee_opr2,
                                                  label0, label1, label2, eps);
    }
  } else if (eps[0] == PropertyType::Int64()) {
    if (eps[1] == PropertyType::Date()) {
      return _make_tc_opr<int64_t, Date>(std::move(src_opr), ee_opr0, ee_opr1,
                                         v_opr1, ee_opr2, label0, label1,
                                         label2, eps);
    } else if (eps[1] == PropertyType::Empty()) {
...
bl::result<ReadOpBuildResultT> TCOprBuilder::Build(
  ...
  if (tc_fusable(plan.plan(op_idx).opr().edge(),
    ...
    auto labels1 =
        gs::runtime::parse_label_triplets(plan.plan(op_idx + 2).meta_data(0));
    auto labels2 =
        gs::runtime::parse_label_triplets(plan.plan(op_idx + 4).meta_data(0));
    ...
    auto opr = make_tc_opr(std::move(src_opr), plan.plan(op_idx).opr().edge(),
                           plan.plan(op_idx + 2).opr().edge(),
                           plan.plan(op_idx + 3).opr().vertex(),
