...
bool expand_vertex_without_predicate_impl<SLVertexColumn>(
    ...
    if (ed_type == PropertyType::Empty()) {
      if (se) {
        return expand_vertex_np_se<grape::EmptyType,
            ...
            DummyPredicate<grape::EmptyType>(), state);
      } else {
        return expand_vertex_np_me_sp<grape::EmptyType,
                                      DummyPredicate<grape::EmptyType>>(
            graph, input, v_tag, label_dirs, DummyPredicate<grape::EmptyType>(),
            state);
      }
      ...
      if (se) {
        return expand_vertex_np_se<int, DummyPredicate<int>>(
            graph, input, v_tag, std::get<0>(label_dirs[0]),
            std::get<1>(label_dirs[0]), std::get<2>(label_dirs[0]),
            DummyPredicate<int>(), state);
      } else {
        return expand_vertex_np_me_sp<int, DummyPredicate<int>>(
            graph, input, v_tag, label_dirs, DummyPredicate<int>(), state);
      }
      ...
      if (se) {
        return expand_vertex_np_se<int64_t, DummyPredicate<int64_t>>(
            graph, input, v_tag, std::get<0>(label_dirs[0]),
            std::get<1>(label_dirs[0]), std::get<2>(label_dirs[0]),
            DummyPredicate<int64_t>(), state);
      } else {
        return expand_vertex_np_me_sp<int64_t, DummyPredicate<int64_t>>(
            graph, input, v_tag, label_dirs, DummyPredicate<int64_t>(), state);
      }
      ...
      if (se) {
        return expand_vertex_np_se<Date, DummyPredicate<Date>>(
            graph, input, v_tag, std::get<0>(label_dirs[0]),
            std::get<1>(label_dirs[0]), std::get<2>(label_dirs[0]),
            DummyPredicate<Date>(), state);
      } else {
        return expand_vertex_np_me_sp<Date, DummyPredicate<Date>>(
            graph, input, v_tag, label_dirs, DummyPredicate<Date>(), state);
      }
    } else {
  ...
  return expand_vertex_np_me_mp<DummyPredicate<Any>>(
      graph, input, v_tag, label_dirs, DummyPredicate<Any>(), state);
}
...
bool expand_vertex_without_predicate_optional_impl<SLVertexColumn>(
  ...
  return expand_vertex_optional_impl<DummyPredicate<Any>>(
      graph, input, v_tag, _label_dirs, DummyPredicate<Any>(), state);
}
template <>
