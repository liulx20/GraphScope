...
class VertexOpr : public IReadOperator {
 public:
  VertexOpr(const physical::GetV& opr) : opr_(opr) {}

  gs::runtime::Context Eval(const gs::runtime::GraphReadInterface& graph,
                            const std::map<std::string, std::string>& params,
                            gs::runtime::Context&& ctx,
                            gs::runtime::OprTimer& timer) override {
    return gs::runtime::eval_get_v(opr_, graph, std::move(ctx), params, timer);
  }
...
class VertexOprBuilder : public IReadOperatorBuilder {
 public:
  ...
  std::pair<std::unique_ptr<IReadOperator>, ContextMeta> Build(
    ...
    int alias = -1;
    if (plan.plan(op_idx).opr().vertex().has_alias()) {
      alias = plan.plan(op_idx).opr().vertex().alias().value();
    }
    ContextMeta ret_meta = ctx_meta;
    ret_meta.set(alias);
    return std::make_pair(
        std::make_unique<VertexOpr>(plan.plan(op_idx).opr().vertex()),
        ret_meta);
  }
