...
#ifndef RUNTIME_EXECUTE_OPS_EDGE_H_
#define RUNTIME_EXECUTE_OPS_EDGE_H_
...
class EdgeExpandOpr : public IReadOperator {
 public:
  EdgeExpandOpr(const physical::EdgeExpand& opr,
  ...
  gs::runtime::Context Eval(const gs::runtime::GraphReadInterface& graph,
    ...
    return gs::runtime::eval_edge_expand(opr_, graph, std::move(ctx), params,
                                         timer, meta_);
  }

 private:
  physical::EdgeExpand opr_;
 ...
 public:
  ...
  std::pair<std::unique_ptr<IReadOperator>, ContextMeta> Build(
    ...
    return std::make_pair(
        std::make_unique<EdgeExpandOpr>(plan.plan(op_idx).opr().edge(),
...
class EdgeExpandGetVOpr : public IReadOperator {
 public:
  EdgeExpandGetVOpr(const physical::EdgeExpand& ee_opr,
                    const physical::GetV& v_opr,
                    const physical::PhysicalOpr_MetaData& meta)
      : ee_opr_(ee_opr), v_opr_(v_opr), meta_(meta) {}

  gs::runtime::Context Eval(const gs::runtime::GraphReadInterface& graph,
                            const std::map<std::string, std::string>& params,
                            gs::runtime::Context&& ctx,
                            gs::runtime::OprTimer& timer) override {
    return gs::runtime::eval_edge_expand_get_v(
        ee_opr_, v_opr_, graph, std::move(ctx), params, timer, meta_);
  }

 private:
  physical::EdgeExpand ee_opr_;
  physical::GetV v_opr_;
  physical::PhysicalOpr_MetaData meta_;
};
 ...
 public:
  ...
  std::pair<std::unique_ptr<IReadOperator>, ContextMeta> Build(
    ...
    if (edge_expand_get_v_fusable(plan.plan(op_idx).opr().edge(),
      ...
      ContextMeta meta = ctx_meta;
      meta.set(alias);
      return std::make_pair(std::make_unique<EdgeExpandGetVOpr>(
                                plan.plan(op_idx).opr().edge(),
                                plan.plan(op_idx + 1).opr().vertex(),
                                plan.plan(op_idx).meta_data(0)),
                            meta);
    } else {
...
#endif  // RUNTIME_EXECUTE_OPS_EDGE_H_