...
#ifndef RUNTIME_EXECUTE_OPS_LIMIT_H_
#define RUNTIME_EXECUTE_OPS_LIMIT_H_
...
class LimitOpr : public IReadOperator {
 public:
  LimitOpr(const algebra::Limit& opr) : opr_(opr) {}

  gs::runtime::Context Eval(const gs::runtime::GraphReadInterface& graph,
                            const std::map<std::string, std::string>& params,
                            gs::runtime::Context&& ctx,
                            gs::runtime::OprTimer& timer) override {
    return gs::runtime::eval_limit(opr_, std::move(ctx));
  }
 ...
 public:
  ...
  std::pair<std::unique_ptr<IReadOperator>, ContextMeta> Build(
    ...
    return std::make_pair(
        std::make_unique<LimitOpr>(plan.plan(op_idx).opr().limit()),
        ctx_meta);
  }
...
#endif  // RUNTIME_EXECUTE_OPS_LIMIT_H_