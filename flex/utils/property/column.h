...
class StringColumn : public ColumnBase {
 public:
  StringColumn(StorageStrategy strategy, size_t width = 64) : width_(width) {}
  ~StringColumn() { close(); }
  ...
  void open(const std::string& name, const std::string& snapshot_dir,
    ...
    // LOG(INFO) << "open file name: " << work_dir << "/" << name << " "
    //         << (int) std::filesystem::exists(work_dir + "/" + name) << "\n";
    extra_buffer_.open(work_dir + "/" + name, false);
    // LOG(INFO) << "open file name: " << work_dir << "/" << name
    //         << " successful\n";
    extra_size_ = extra_buffer_.size();
...
template <typename INDEX_T>
void StringMapColumn<INDEX_T>::init_meta_map() {
...
void StringMapColumn<INDEX_T>::open(const std::string& name,
  ...
  // meta_map_->init_with_resize(name + ".map_meta", snapshot_dir, work_dir,
