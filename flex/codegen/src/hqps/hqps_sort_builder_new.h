...
static constexpr const char* SORT_OP_TEMPLATE_STR =
    "%1%OrderBy::order_by_with_limit(txn, %2%,%3%,%4%,%5%);";

std::pair<std::string, bool> sort_pair_pb_to_order_pair(
  ...
  if (pair.key().has_property()) {
    ...
    if (sort_property.has_label()) {
      ...
      if (sort_prop_name == "id") {
        // TODO expr id
        ss << "VertexIdPathAccessor "
           << "accessor" << std::to_string(idx) << "(txn, "
           << ctx.GetCurCtxName() << ", " << pair.key().tag().id() << ");\n";
      } else {
        ss << "VertexPropertyPathAccessor<" << sort_prop_type << "> "
           << "accsessor" << std::to_string(idx) << "(txn, "
           << ctx.GetCurCtxName() << ", " << pair.key().tag().id() << ", \""
           << pair.key().property().key().name() << "\");\n";
      }
    } else {
    ...
    sort_prop_type =
        single_common_data_type_pb_2_str(pair.key().node_type().data_type());
    ss << "ContextValueAccessor<" << sort_prop_type << "> "
       << "accessor" << std::to_string(idx) << "(txn, " << ctx.GetCurCtxName()
       << ", " << pair.key().tag().id() << ");\n";
  }

  return {ss.str(), desc_flag};
  // boost::format formater(ORDERING_PAIR_TEMPLATE_STR);
  // formater % sort_order_str % real_key_tag_id % sort_prop_type %
  // sort_prop_name; return formater.str();
}
...
class SortOpBuilder {
 public:
  ...
  std::string Build() const {
    ...
    std::string comparer_str;
    {
      std::stringstream ss;

      std::stringstream comparer_ss;
      comparer_ss << "GeneralTemplatedComparer(std::make_tuple(";
      for (size_t i = 0; i < sort_pairs_.size(); ++i) {
        auto val = sort_pair_pb_to_order_pair(ctx_, sort_pairs_[i], i);
        ss << val.first;
        if (val.second) {
          comparer_ss << "DescWrapper(std::move(accessor" << i << "))";
        } else {
          comparer_ss << "AscWrapper(std::move(accessor" << i << "))";
        }
        if (i + 1 != sort_pairs_.size()) {
          comparer_ss << ", ";
        }
      }
      comparer_ss << "))";
      comparer_str = comparer_ss.str();
    ...
    formater % ordering_pairs_str % prev_ctx_name % comparer_str % range_lower %
        range_upper;
